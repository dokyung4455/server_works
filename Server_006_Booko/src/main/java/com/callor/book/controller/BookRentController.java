...
public class BookRentController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// rent/seq 라고 요청을 하면 subPath 에는 /seq라는 문자열이 담길것이다.
		...
		if (subPath.equals("/seq")) {
			 ...
			 * 
			 * Web App Service를 구현하기 위하여 Req,Res를 처리하는 여러가지 기능을 구현해야 하는데 그러한 기능을 미리 구현해 놓았기
			 * 때문에 ServletContext를 getter 하는 것만으로 충분하다.
			 * 
			 * DB 등으로 부터 조회된 데이터를 Web 에게 응답하고자 할때 쉬운 방법으로 전달할 수 있도록 하는 기능이 미리 구현되어 있다.
			 */
