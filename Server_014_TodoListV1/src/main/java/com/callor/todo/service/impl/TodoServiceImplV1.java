...
public class TodoServiceImplV1 implements TodoService {
	 ...
	 * 메타데이터
	 * 
	 * 객체(Entity,Relation) DataBase를 만들고 Table을 생성한 후 Insert를 수행하면 실제상황의 데이터가 Table에
	...
	public List<Map<String, Object>> SelectAll() {
		...
		try {
			...
			return tdList;
			/*
			 * ResultSetMetaData md = rSet.getMetaData(); // 칼럼갯수를 가져오고 int count =
			 * md.getColumnCount(); // 몇개인지 추출
			 * 
			 * System.out.println("=".repeat(50)); for(int i = 0 ; i < count ; i++) {
			 * System.out.print(md.getColumnName(i+1) + "\t"); // 칼럼네임의 위치값 td_seq,
			 * td_sdate, td_stime, td_doit, td_edate, td_etime } System.out.println();
			 ...
			 * ; i ++) { System.out.print(rSet.getString(i+1) + "\t"); }
			 * System.out.println();
			 * 
			 * }
			 */
	...
	public Integer update(Map<String, Object> tdVO) {
		...
		try {
			pStr = dbConn.prepareStatement(sql);
			pStr.setObject(1, tdVO.get(DBinfo.td_sdate));
			pStr.setObject(2, tdVO.get(DBinfo.td_sdate));
			pStr.setObject(3, tdVO.get(DBinfo.td_doit));
