...
public class BookController extends HttpServlet {
	...
	/*
	 * WAS 관련 프로젝트를 수행할때 클래스를 만들고 서버를 한번 Run 한 후에 다시 클래스 코드를 변경하면 Tomcat이 자동으로 재시작
	 * 된다. 그때 Tomcat이 내부적으로 클래스의 정보를 참조하는 일종의 Key값
	 */
...
//		db/book/* 의 pattern으로 요청을 하면 * 부분에 추가된 sub Path를 추출하여 여러가지 요청을 처리 할 수 있다.
//		이때 추가된 sub Path를 추출할때는 req.getPathInfo() 를 사용하여 추출한다.
		...
		if(subPath.equals("/select")) {
			...
			BookVO bookVO = bService.findeById(isbn);
			if(bookVO == null) {
				out.println("찾는 데이터 없음");
			}
...
//		out.println(req.getContextPath());
	...
	private void callService() {
		...
		// 도서 1개 정보 조회
		BookVO retVO = bService.findeById("9175484641854");

		// 도서정보 변경
		bService.update(bookVO);

		// 도서정보 데이터 1개 삭제
		bService.delete("9175484641854");
